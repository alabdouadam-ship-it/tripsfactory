...
/// We test a widget that uses the same .select() pattern as HomeScreen (companyStatus, isClientMode).
void main() {
  testWidgets('home-style consumer stays under rebuild budget on profile update', (tester) async {
    ...
    int rebuildCount = 0;
    final profileState = StateProvider<Profile?>((ref) => const Profile(id: 'u1', fullName: 'Test', companyStatus: 'approved'));
          ...
          home: RebuildCounter(
            ...
            child: Consumer(
              builder: (context, ref, _) {
                ...
                ref.watch(currentUserProfileProvider.select((p) => p.value?.companyStatus));
    ...
    container.read(profileState.notifier).state = const Profile(id: 'u1', fullName: 'Test', companyStatus: 'approved');
    await tester.pump();
