...
void main() {
  ...
  group('ShipmentStatus FSM (Handshake Guards)', () {
    test('pending transitions', () {
      final status = ShipmentStatus.pending;
      expect(status.canTransitionTo(ShipmentStatus.inCommunication), isTrue);
      expect(status.canTransitionTo(ShipmentStatus.accepted), isTrue);
      expect(status.canTransitionTo(ShipmentStatus.cancelled), isTrue);

      // Invalid
      expect(status.canTransitionTo(ShipmentStatus.inTransit), isFalse);
    });

    test('accepted transitions', () {
      final status = ShipmentStatus.accepted;
      expect(status.canTransitionTo(ShipmentStatus.pickedUp), isTrue);
      expect(status.canTransitionTo(ShipmentStatus.inTransit), isTrue);
      expect(status.canTransitionTo(ShipmentStatus.cancelled), isTrue);

      // Invalid
      expect(status.canTransitionTo(ShipmentStatus.completed), isFalse);
    });

    test('pickedUp transitions', () {
      final status = ShipmentStatus.pickedUp;
      expect(status.canTransitionTo(ShipmentStatus.inTransit), isTrue);
      expect(status.canTransitionTo(ShipmentStatus.delivered), isTrue);
      expect(status.canTransitionTo(ShipmentStatus.completed), isTrue);

      // Invalid
      expect(status.canTransitionTo(ShipmentStatus.cancelled), isFalse);
    });

    test('inTransit transitions', () {
      final status = ShipmentStatus.inTransit;
      expect(status.canTransitionTo(ShipmentStatus.delivered), isTrue);
      expect(status.canTransitionTo(ShipmentStatus.completed), isTrue);
    });
    ...
    test('terminal states', () {
      expect(
        ShipmentStatus.completed.canTransitionTo(ShipmentStatus.cancelled),
        isFalse,
      );
      expect(
        ShipmentStatus.cancelled.canTransitionTo(ShipmentStatus.pending),
        isFalse,
      );
    });
  });
